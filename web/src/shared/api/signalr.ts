...
class RealtimeClient {
  ...
  private accessTokenFactory: () => string | null | Promise<string | null> = () => null

  public readonly state: Ref<HubConnectionState> = ref(HubConnectionState.Disconnected)

  configure(accessTokenFactory: () => string | null | Promise<string | null>) {
  ...
  async connect() {
    ...
    this.connection = new HubConnectionBuilder()
      .withUrl('/hubs/user', {
        accessTokenFactory: async () => (await this.accessTokenFactory()) ?? ''
      })
